...
def comp_move(defboard):
    gain = 0
    realx = random.randint(6*n,6*n+5)
    ...
    for defx in range(6*n,6*n+6):
        ...
        while cdefboard[cdefx] in range(2,4) and cdefx not in range(6*n,6*n+6):
...
def move(n, defboard):
    while True:
        ...
        defx = comp_move(defboard)
        print(f'the computer{n} chose position {defx}')
...
if board[12] > board[13]:
    winner = 'comp_player 1'
elif board[13] > board[12]:
    winner = 'comp_player 2'
